document.addEventListener("DOMContentLoaded", () => {
  ...
  // Ghi nhận một bản ghi "Đang xử lý" vào myTickets trong 15 phút giữ chỗ
  try {
    const COUNTDOWN_KEY = "buyTicketCountdownEnd";
    const expiresAt = Number(sessionStorage.getItem(COUNTDOWN_KEY));
    const eventId = localStorage.getItem("selectedEventId") || null;
    if (expiresAt && Date.now() < expiresAt) {
      const holdIdKey = "currentProcessingHoldId";
      let holdId = sessionStorage.getItem(holdIdKey);
      if (!holdId) {
        holdId = `H${Date.now()}${Math.floor(Math.random()*1000)}`;
        sessionStorage.setItem(holdIdKey, holdId);
      }
      const entry = {
        holdId,
        eventId,
        eventName: ticketInfo.eventName || null,
        zoneName: ticketInfo.zoneName || null,
        quantity: Number(ticketInfo.quantity) || 1,
        price: Number(ticketInfo.price) || 0,
        total: Number(ticketInfo.total) || (Number(ticketInfo.price)||0) * (Number(ticketInfo.quantity)||1),
        status: "Đang xử lý",
        createdAt: new Date().toISOString(),
        expiresAt
      };
      const myTickets = JSON.parse(localStorage.getItem("myTickets") || "[]");
      const exists = myTickets.some(t => t.holdId === holdId);
      if (!exists) {
        myTickets.push(entry);
